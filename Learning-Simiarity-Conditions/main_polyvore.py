...
from triplet_image_loader import TripletPolyvoreImageLoader
...
parser.add_argument('--name', default='polyvore', type=str,
...
parser.add_argument('--text_dim', type=int, default=600, metavar='N',
                    help='how many dimensions in text feature')
...
def main():
    ...
    tnet = Tripletnet(args, csn_model, args.num_concepts, args.text_dim, criterion)
    if args.cuda:
        tnet.cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        save_checkpoint(args.model_path, {
            'epoch': epoch + 1,
...
def test(test_loader, tnet, plot_roc=False, save_path=None):
    ...
    for batch_idx, images in enumerate(test_loader):
        ...
        embeddings.append(tnet.get_image_embedding(images).data)
    
    embeddings = torch.cat(embeddings)
    auc = test_loader.dataset.test_compatibility(embeddings, plot_roc, save_path)
